...
while true
    ...
    if (dropOffPointReached == true)
        dropOffPointReached = MazeRunnerFunctions.manualControl(brick, key);
        continue;

    else
        [dropOffPointReached, blueCount, greenCount, redLineCount] = MazeRunnerFunctions.autoControl(brick, distance, blueCount, greenCount, redLineCount, color);
        continue;
    end
